...
Customizing the Collidable Area:
    ...
    [bounds] ObjectProperty (list or dict) ([[[(0, 0), (1, 0), (1, 1), (0, 1)], [0, 2]]])
...
class Rotabox2(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        super(Rotabox2, self).__init__(**kwargs)
        ...
        '''Enable bounds visualization (for testing).'''
    ...
    def add_widget(self, widget, index=0, **kwargs):
        ...
        super(Rotabox2, self).add_widget(widget, index)
    ...
    def prepare(self):
        ...
        if not self.image:
            ...
            if self.child and isinstance(self.child, Image):
                self.image = self.child
        if self.image:
            self.image.allow_stretch = True
            # self.image.texture.mag_filter = 'nearest'
            self.image.bind(source=self.scale_me)
    ...
    def scale_me(self, *args):
        ...
        # Adjusting the image's or child's size
        if self.image:
        ...
        if self.custom_bounds:
            ...
            self.update_bounds(self.polygons, True, self.radiangle)
    ...
    def update_bounds(self, polygons, motion=False, angle=0.):
            ...
            for alien in alien_points:
                for pol in self.polygons:
                    ...
                    for index in pol.check_ids:
                        ...
                        if pol.neg_dir:
                            ray1, ray2 = ray2, ray1
                        # Normalizing comparison if the cartesian circle's seam
                        ...
                        '''Returning the number of the colliding polygon instead 
                        of True. (index + 1, to always evaluate to True).'''
                        if pol.check_ids:  # Polygons without checkpoints skip 
                                           # the 'loop, causing false positives.
...
if __name__ == '__main__':
    ...
    Builder.load_string('''
#:import Scatter2 scatter_for_rotabox
<Root>:
    blue: blue
    red: red
    Scatter2:
