...
# -------------------------- AI -------------------------
    ...
    def trace_image(self):
        ...
        print 'trace', image.x, image.y, image.texture.size, image._coreimage.size
        ...
        for x in xrange(0, width, step):
            ...
            for y in xrange(0, height, step):
                pixel = image._coreimage.read_pixel(x, height - y - 1)
                if not opaque and pixel[-1]:
                    self.points.append([x, y])
                    opaque = True
                    kept += 1
                elif opaque and not pixel[-1]:
                    a = x if y else x - 1
                    b = y - 1 if y else height - 1
                    self.points.append([a, b])
                    opaque = False
                    kept += 1
                elif y == height - 1 + (height - height // step * step) and kept % step:
                    self.points.append([x, y])
                    opaque = False

        print 'a', len(self.points)
        ...
        for y in xrange(0, height, step):
            ...
            for x in xrange(0, width, step):
                ...
                if not opaque and pixel[-1]:
                    ...
                    b = y if x else y - 1
                    a = x - 1 if x else width - 1
                    if [a, b] not in self.points:
                        self.points.append([a, b])
                    opaque = False
                    kept += 1
                elif x == width - 1 + (width - width // step * step) and kept % step:
                    self.points.append([x, y])
                    opaque = False
        print 'b', len(self.points)

        with image.canvas.after:
            Color(1, 0, 1, 1)
            for x, y in self.points:
                Line(circle=(x + image.x, y + image.y, .5))
        #     # Color(.3, .6, 0, 1)
        #     # Line(points=(n for point in self.points
        #     #              for n in (point[0] + image.x, point[1] + image.y)))
    ...
    def link_points(self):
        ...
        while points:
            self.linked.append(points.pop(points.index(point)))

            for other in points:
                dist = self.calc_ray(point, other)[1]
                if dist > 1.5:  # -----------------possible variable
                    dists.append(dist)
                    possibles.append(other)
                else:
                    points.remove(other)

            dist0 = self.calc_ray(point, self.linked[0])[1]

            if dists:
                dist = min(dists)
                if dist <= dist0 or self.linked[0] is point:
                    point.append(dist)
                    point = possibles[dists.index(dist)]
                    del dists[:]
                    del possibles[:]
                    continue
                del dists[:]
            ...
            point.append(dist0)
            break
        print 'c', len(self.linked)
        del self.points[:]

        image.canvas.after.clear()
        with image.canvas.after:
            Color(1, 0, 1, 1)
            for x, y, d in self.linked:
                Line(circle=(x + image.x, y + image.y, .5))
            Color(.5, .5, 0, 1)
            Line(points=(n for point in self.linked
                         for n in (point[0] + image.x, point[1] + image.y)),
                 close=True)
    ...
    def find_regions(self):
        image = self.sprite.image
        # print 'linked', len(self.linked), self.linked[-1]

        for i, point in enumerate(self.linked):
            # print i, len(self.linked)
            angle, distance = self.calc_ray(point,
                                            self.linked[(i + 1) % len(self.linked)])
            point[2] = distance
            point.append(angle)
        ...
        for i, point in enumerate(self.linked):
            ...
            # diffs.append(diff)
            # if len(diffs) > 2:
                # fluct = sum(diffs) / len(diffs)
                # diffs.popleft()
                # diffs.popleft()
            if direc != direction:
                diff2 = self.linked[(i + 1) % len(self.linked)][3] - point[3]
                if diff2 > 10:
                    dir2 = 1
                elif diff2 < -10:
                    dir2 = -1
                ...
                if dir2 != direction:
                    self.essentials.append(point)
                else:
                    # self.noise.append(point)
                    self.noise.append(self.linked[(i + 1) % len(self.linked)])
            angle = point[3]
        print 'regions', len(self.noise)
        ...
        with image.canvas.after:
            ...
            color = Color(.5, .7, 0, 1)
            Line(points=(n for dot in dots for n in dot))
            color = Color(.5, .5, .5, 1)
            Line(points=(dots[0][0], dots[0][1], dots[0][0] + 1000, dots[0][1]))

            color = Color(1, .5, 1, 1)
            for x, y, d, a in self.linked:
                Line(circle=(x + image.x, y + image.y, .5))
            color = Color(.5, .7, 0, 1)
            Line(points=(n for point in self.linked
                         for n in (point[0] + image.x, point[1] + image.y)),
    ...
    def filter_points(self):
        ...
        for i, point in enumerate(self.linked):
            point.append(atan2((self.linked[(i + 1) % len(self.linked)][1] - point[1]),
                               (self.linked[(i + 1) % len(self.linked)][0] - point[0]))
                         % 6.283)

        print 'd', len(self.linked), self.linked[-1]

        angle = self.linked[0][3]
        points2 = [self.linked[0]]
        noise = []
        for i, point in enumerate(self.linked):
            print 'filter', degrees(point[3])
            if point in noise:
                noise.remove(point)
                continue
            # if point[3] != angle:
            if (abs(point[3] - angle) > .7
            and (point[2] > 10
            or abs(self.linked[(i + 1) % len(self.linked)][3] - angle) > .5)):
                # if point[2] < self.simplicity:
                #         self.find_noise(linked, noise, i)
        ...
        for i, point in enumerate(points2):
            point[2] = (((points2[(i + 1) % len(points2)][0] - point[0]) ** 2
                        + (points2[(i + 1) % len(points2)][1] - point[1]) ** 2)
                        ** .5)

        for i, point in enumerate(points2):
            point[3] = (atan2((points2[(i + 1) % len(points2)][1] - point[1]),
                               (points2[(i + 1) % len(points2)][0] - point[0]))
                        % 6.283)
        ...
        for i, point in enumerate(points2):
            ...
            # if (abs(point[3] - angle) > .7):
            # and (point[2] > 3
            # or abs(points2[(i + 1) % len(points2)][3] - angle) > .7)):
                # if point[2] < self.simplicity:
                #         self.find_noise(points2, noise, i)
        ...
        with image.canvas.after:
            Color(1, 0, 1, 1)
            for x, y, d, a in points3:
                Line(circle=(x + image.x, y + image.y, .5))
            Color(.3, .6, 0, 1)
            Line(points=(n for point in points3
                         for n in (point[0] + image.x, point[1] + image.y)),
                 close=True)

    def find_noise(self, points, noise, i):
        if (points[(i + 1) % len(points)][2]
                < self.simplicity
                and abs(points[i][3] - points[(i + 1) % len(points)][3])
                > self.simplicity * .03):
            noise.append(points[(i + 1) % len(points)])
            self.find_noise(points, noise, (i + 1) % len(points))
